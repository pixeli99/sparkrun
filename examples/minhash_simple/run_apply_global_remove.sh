...
LOG_FILE="${LOG_PATH}/minhash-simple-apply-${TIMESTAMP}.log"
ERR_FILE="${LOG_PATH}/minhash-simple-apply-${TIMESTAMP}.err"
...
echo "------------------------------------------------"
echo "MinHash simple apply_global_remove config:"
...
spark-submit \
    ...
    tools/minhash_simple/apply_global_remove.py \
    "${CONFIG_PATH}" \
    > "${LOG_FILE}" 2> "${ERR_FILE}"
